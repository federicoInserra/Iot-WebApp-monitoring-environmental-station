...
static int generate_values(int max_range)
{   
...
{   
    ...
    // Create the payload to send to TTN
    sprintf(detections, "{\"stN\": \"environmentalStation2\", \"tmp\": \"%u\", \"hum\": \"%u\",\"wd\": \"%d\", \"wi\": \"%d\", \"rh\": \"%d\"}",
     (temperature/10), (humidity/10), wind_direction, wind_intensity, rain_height);
    
    printf("Sending: %s\n", detections);
    /* Try to send the message */
    uint8_t ret = semtech_loramac_send(&loramac,
                                       (uint8_t *)detections, strlen(detections));
...
int main(void)
{
    ...
    if(hts221_power_on(&hts221) != HTS221_OK){
        puts("Sensor power on failed!");
        return 1;

    }

    if(hts221_set_rate(&hts221, hts221.p.rate) != HTS221_OK){
        puts("Unable to start sensors!");
        return 1;
